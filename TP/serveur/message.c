...
*/
inline struct sockaddr_in ___message_whois_next_server___(struct sockaddr_in
							  serv_addr)
{
...
inline int ___message_connect_to___(struct sockaddr_in server_info)
{
	...
	my_server_ptr->precServeur = server_info;
    ...
    sockServer = ___connect2server___(my_server_ptr->suivServeur);
	...
	if (close(sockServer) < 0) {
		printf
		    ("___message_connect_to__:Echec de la deuxieme deconnexion du serveur\n");
		exit(-1);
	}
	return 1;
}
...
int ____message_transfer_DHT_to_next_server____()
{
	...
	envoyerOrigine(FROM_SERVEUR, sock_server);
	envoyerTypeMessage(TRANSFER_DHT, sock_server);
...
int message_connect_2_server(char *ip, uint32_t port)
{
	...
	while (1) {
		...
		if (serv_addr.sin_port == htons(port)) {
			...
			___message_connect_to___(server_most_charged.
						 identifiant);
...
int message_quit()
{
	...
	if (my_server_ptr->serv_addr.sin_port !=
	    my_server_ptr->suivServeur.sin_port) {

        ____message_quit____();
...
int ____message_quit____(){

    struct sockaddr_in suivServer,precServer;
    serveur_t *my_server_ptr = get_my_server();
	suivServer = my_server_ptr->suivServeur;
	precServer = my_server_ptr->precServeur;
	socket_t sock_server;
	/*
	 * connexion au serveur
	 */
	sock_server = ___connect2server___(suivServer);
	envoyerOrigine(FROM_SERVEUR, sock_server);
	envoyerTypeMessage(QUIT, sock_server);
    
    envoyerOctet(0,sock_server);
    envoyerSockAddr(suivServer,sock_server);
	...
	sock_server = ___connect2server___(precServer);
	envoyerOrigine(FROM_SERVEUR, sock_server);
	envoyerTypeMessage(QUIT, sock_server);
    
    envoyerOctet(1,sock_server);
    envoyerSockAddr(precServer,sock_server);
